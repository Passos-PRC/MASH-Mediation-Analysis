...
prepare_mediation_data <- function(dataset, study_meta, study_meta_mod,
                                   outcome_col = "event1",
                                   n_col       = "n(PO1)") {
  stopifnot(
  ...
  study_meta_mod <- study_meta_mod %>% mutate(study_id = clean_id(study_id))
...
prepare_mediation_data_1 <- function(dataset, study_meta, study_meta_mod) {
  prepare_mediation_data(dataset, study_meta, study_meta_mod,
                         outcome_col = "event1", n_col = "n(PO1)")
}

prepare_mediation_data_2 <- function(dataset, study_meta, study_meta_mod) {
  prepare_mediation_data(dataset, study_meta, study_meta_mod,
                         outcome_col = "event2", n_col = "n(PO2)")
}
...
compute_point_estimates_bayes <- function(contrasts, contrasts_long, all_groups,
  ...
  for (i in seq_len(nrow(all_groups))) {
    ...
    prop_draws     <- ifelse(abs(tot_s) > 1e-8,
                             pmax(0, pmin(1, indirect_draws / tot_s)), NA_real_)
    ...
    prp_smry <- smry_draws(prop_draws)
    ...
    b_est <- b_summary$estimate[1]; b_lo <- b_summary$lower[1]; b_hi <- b_summary$upper[1]

    results[[i]] <- tibble(
      group = group_label, k = nrow(df_sub), n_studies = n_studies, total_n = total_n,
      a = a_smry$m, a_se = a_smry$se, a_lower = a_smry$l, a_upper = a_smry$u,
      BF10_a = a_fit$summary$BF10[1],
      b = b_est, b_se = b_summary$se[1], b_lower = b_lo, b_upper = b_hi,
      BF10_b = b_summary$BF10[1],
      total = tot_smry$m, total_se = tot_smry$se, total_lower = tot_smry$l, total_upper = tot_smry$u,
      BF10_total = total_fit$summary$BF10[1],
      indirect = ind_smry$m, indirect_lower = ind_smry$l, indirect_upper = ind_smry$u,
      BF10_indirect = BF10_indirect,
      direct = dir_smry$m, direct_lower = dir_smry$l, direct_upper = dir_smry$u,
      prop_indirect = prp_smry$m, prop_indirect_pct = 100 * prp_smry$m,
      prop_lower = prp_smry$l, prop_upper = prp_smry$u,
      indirect_rd = ind_smry$m, direct_rd = dir_smry$m, total_rd = tot_smry$m,
      indirect_OR = ind_smry$m, direct_OR = dir_smry$m, total_OR  = tot_smry$m,
      logOR_resp  = tot_smry$m,
      b_RD_per5 = b_est * 5, b_RD_per5_lower = b_lo * 5, b_RD_per5_upper = b_hi * 5,
      b_OR_per5 = b_est * 5, b_OR_per5_lower = b_lo * 5, b_OR_per5_upper = b_hi * 5
    )
  }
  bind_rows(results)
}
...
make_forest_plot <- function(med, desired_order, b_global) {
  ...
  b_label <- coalesce(
    if ("b_RD_per5" %in% names(b_global)) b_global$b_RD_per5 else NA_real_,
    if ("b_OR_per5" %in% names(b_global)) b_global$b_OR_per5 else NA_real_
  )

  indirect_col <- if ("indirect_rd" %in% names(med)) "indirect_rd" else "indirect"
  direct_col   <- if ("direct_rd"   %in% names(med)) "direct_rd"   else "direct"

  forest_data <- med %>%
    dplyr::select(
      group, a, a_lower, a_upper, total_n,
      indirect = all_of(indirect_col), indirect_lower, indirect_upper,
      direct   = all_of(direct_col),   direct_lower,   direct_upper
    ) %>%
  ...
  p_forest <- ggplot(forest_data,
    ...
    facet_wrap(~ effect, scales = "free_x", nrow = 1,
      labeller = as_labeller(c(
        ...
        direct   = "Direct effect (weight-independent)"
      ))) +
    scale_size_continuous(name = "Total sample size", range = c(2, 8), labels = scales::comma) +
    theme_minimal(base_size = 14) +
    theme(panel.grid.minor = element_blank(), strip.text = element_text(face = "bold"),
          axis.text.y = element_text(face = "bold"), panel.spacing = unit(1.5, "lines")) +
    labs(
      title    = "Mediation effects by drug class and key trial subgroups",
      subtitle = paste0("Global b-path (posterior mean): RD ", round(b_label, 4),
                        " per 5 pp weight loss | Bayesian 95% CrI"),
      x = "Estimate (95% CrI)", y = NULL
    )

  prop_data <- med %>%
    mutate(group = factor(group, levels = levels(forest_data$group)), prop = prop_indirect_pct)
  prop_max  <- if (all(is.na(prop_data$prop))) 1 else max(prop_data$prop, na.rm = TRUE) * 1.2

  p_prop <- ggplot(prop_data, aes(x = prop, y = group)) +
    geom_col(fill = "#56382D", width = 0.6, na.rm = TRUE) +
    geom_vline(xintercept = 0, linetype = "dashed") +
    geom_text(aes(label = ifelse(is.na(prop), "NA", paste0(round(prop, 1), "%"))),
              hjust = -0.1, size = 4) +
    scale_x_continuous(limits = c(0, prop_max), labels = function(x) paste0(x, "%")) +
    theme_minimal(base_size = 14) +
    theme(axis.title.y = element_blank(), axis.text.y = element_blank(),
          axis.ticks.y = element_blank(), panel.grid.minor = element_blank()) +
    labs(title = "% mediated", x = NULL)

  list(
    forest_data = forest_data,
    p_forest    = p_forest,
    p_prop      = p_prop,
    p_final     = p_forest + p_prop + plot_layout(widths = c(3, 1))
  )
}

make_bubble_plot <- function(contrasts, b_global) {
  bubble_data <- contrasts %>%
    mutate(weight = 1 / var_rd_resp, weight_scaled = scales::rescale(sqrt(weight), to = c(2, 10)))

  b_label <- coalesce(
    if ("b_RD_per5" %in% names(b_global)) b_global$b_RD_per5 else NA_real_,
    if ("b_OR_per5" %in% names(b_global)) b_global$b_OR_per5 else NA_real_
  )
  yvar <- if ("rd_resp" %in% names(bubble_data)) "rd_resp" else "logOR_resp"

  ggplot(bubble_data,
      aes(x = wt_loss_eff, y = .data[[yvar]], size = weight_scaled, color = drug_class)) +
    ...
    labs(
      title    = "Trial-level association between weight loss and histologic response",
      subtitle = paste0("Each point = treatment contrast; bubble size \u221d inverse-variance weight\n",
                        "Global b-path slope (posterior mean): RD ", round(b_label, 4),
                        " per 5 pp weight loss"),
      x     = "Weight loss vs placebo (percentage points)",
      y     = "Absolute risk difference of histologic response",
      color = "Drug class", size = "Weight"
    )
}
